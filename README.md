...
我现在下载了几个大概几平方公里的地图，glb格式，可以用blender看到建筑物的模型的。但这个文件中不包含建筑物类型和名称，所以不知道建筑物的材料和介电常数电导率。于是我想到可以从网上查一下建筑物的类型和名称，就是网页上看到的地图中的建筑物名称。现在我在用AI编这个程序，把模型中的每一个建筑物的名字材料从地图上找出来。
