...
def point_near_road(lon, lat, road, origin_lon, origin_lat, extra_margin=0.5):
    coords = road["coords"]
    if len(coords) < 2:
        return False
...
def terrain_cell_excluded(lon, lat, exclusion_polygons, exclusion_roads, origin_lon, origin_lat):
    ...
    for road in exclusion_roads:
        if point_near_road(lon, lat, road, origin_lon, origin_lat):
            return True
...
def main():
    ...
    if dem:
        ...
        positions, normals, indices = build_terrain_mesh(
            ...
            exclusion_roads=collector.roads,
        )
    else:
