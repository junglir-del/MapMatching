...
def build_area_mesh(coords, origin_lon, origin_lat, y=0.01, dem=None, terrain_follow=True):
    ...
    if dem and terrain_follow:
        ...
        sampled = [dem.sample(lon, lat, default=0.0) for lon, lat in ring]
        water_height = sum(sampled) / len(sampled) if sampled else 0.0
        heights = [water_height + y for _ in ring]
    else:
...
def build_terrain_mesh(
    ...
    print(f"Generating DEM terrain mesh: {grid_size} x {grid_size} cells")
    ...
    for row in range(grid_size):
        for col in range(grid_size):
            ...
            while percent >= next_progress_percent and next_progress_percent <= 100:
                print(f"DEM terrain progress: {next_progress_percent}%")
                next_progress_percent += 5
    ...
    print("DEM terrain progress: mesh normals")
    normals = compute_vertex_normals(positions, indices)
    return positions, normals, indices
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=-1.0,
        help="Vertical offset in meters applied only to the generated DEM terrain mesh. Default: -1.0",
    )

    parser.add_argument(
        "--generate-dem-terrain",
        action="store_true",
        help="Generate an extra DEM terrain mesh. By default DEM is only used to place OSM features at elevation.",
    )
    ...
    ground_name = "dem_terrain" if dem and args.generate_dem_terrain else "bbox_ground"
    ground_feature = {
        "osm_type": "generated",
        "id": ground_name,
    ...
    if dem and args.generate_dem_terrain:
        raw_terrain_exclusion_polygons = [
            building["coords"] for building in collector.buildings
        ] + [
            area["coords"] for area in collector.areas
        ]
        terrain_exclusion_polygons = prepare_terrain_exclusion_polygons(
            raw_terrain_exclusion_polygons
        )
        terrain_exclusion_roads = prepare_terrain_exclusion_roads(
            collector.roads,
            origin_lon,
            origin_lat,
        )
        print(
            "Prepared terrain clipping: "
            f"{len(terrain_exclusion_polygons)} polygons, "
            f"{len(terrain_exclusion_roads)} roads"
        )
        positions, normals, indices = build_terrain_mesh(
            ...
            exclusion_polygons=terrain_exclusion_polygons,
            exclusion_roads=terrain_exclusion_roads,
        )
    elif not dem:
    ...
    if positions and indices:
        ...
        if dem and args.generate_dem_terrain:
            ...
            metadata["terrain_clipping"] = "conservative_five_point_exclusion"
    ...
    for area in collector.areas:
        ...
        positions, normals, indices = build_area_mesh(
            ...
            origin_lat,
            dem=dem,
            terrain_follow=(feature_type != "water"),
        )
    ...
    extras_doc = {
        ...
        "dem_terrain_generated": bool(dem and args.generate_dem_terrain),
        "remaining_ground_generated": not dem,
        "remaining_ground_material": "soil",
        "unmodeled_background_material": "soil" if dem and not args.generate_dem_terrain else None,
        "unmodeled_background_note": "No ground mesh is generated in DEM mode unless --generate-dem-terrain is used; remaining black/background areas should be treated as inferred soil." if dem and not args.generate_dem_terrain else None,
        "terrain_grid": args.terrain_grid if dem and args.generate_dem_terrain else None,
        "terrain_offset_m": args.terrain_offset if dem and args.generate_dem_terrain else None,
        "terrain_clipping": "conservative_five_point_exclusion" if dem and args.generate_dem_terrain else None,
