...
def main():
    ...
    for building in collector.buildings:
        ...
        metadata["derived"]["gltf_node_index"] = len(writer.nodes)
        metadata["derived"]["vertex_count"] = len(positions)
        metadata["derived"]["triangle_count"] = len(indices) // 3
    ...
    metadata_doc = {
        ...
        "material_property_note": "Each building has osm and derived sections. osm.type, osm.id, and osm.tags identify the original OSM object and keep its tags unchanged. derived contains calculated or inferred values, including inferred_material, dielectric_constant, and conductivity_s_per_m.",
